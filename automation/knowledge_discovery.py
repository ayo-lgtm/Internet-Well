...
REGISTRY = ROOT / "integrations" / "knowledge" / "structured-sources.json"
...
def plan(query: str, language: str | None = None):
    ...
    if not ranked:
        ranked = [{"source": s["id"], "score": 1, "pin": s["pin"], "extract": s["extract"]} for s in sources]
    return {
        "query": query,
...
def main():
    ...
    args=p.parse_args()
    print(json.dumps(registry() if args.cmd=="sources" else plan(args.query,args.language), indent=2))
